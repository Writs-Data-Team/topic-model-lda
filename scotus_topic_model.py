...
for k in topic_nums:
    ...
    topic_assignments[k] = get_dominant_topic(lda, corpus)

# Add topic assignment columns
for k in topic_nums:
    df[f'topic_k{k}'] = topic_assignments[k]

# Save coherence plot
plt.figure(figsize=(6,4))
plt.plot(list(coherence_scores.keys()), list(coherence_scores.values()), marker='o')
plt.title("Coherence Score vs Number of Topics")
plt.xlabel("Topics"); plt.ylabel("Coherence (c_v)")
plt.grid(True)
plt.tight_layout()
plt.savefig(f"data/coherence_vs_topics.png") 
plt.close()
print("Saved coherence plot to data/coherence_vs_topics.png")

# Save all topic assignments
df.to_parquet(f'data/petitions_with_topics_allk.parquet', index=False)
print(f"Saved parquet with topic assignments to data/petitions_with_topics_allk.parquet")